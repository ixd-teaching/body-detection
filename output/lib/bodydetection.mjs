...
function createBodyFromObject(bodyObject) {
    return new Body(bodyObject.id, bodyObject.bodyParts2D.map(bp => new BodyPart2D({ name: bp.name, position: bp.position, speed: bp.speed, confidenceScore: bp.confidenceScore })), bodyObject ? bodyObject.bodyParts3D.map(bp => new BodyPart3D({ name: bp.name, position: bp.position, speed: bp.speed, confidenceScore: bp.confidenceScore })) : null, bodyObject.deviceId);
}
...
class Bodies {
    ...
    getDistanceBetweenBodyParts2D(id1, bodyPartName1, id2, bodyPartName2) {
        return calcDistance2D3D(this.getBody(id1).getBodyPart2D(bodyPartName1).position, this.getBody(id2).getBodyPart2D(bodyPartName2).position);
    }
    getDistanceBetweenBodyParts3D(id1, bodyPartName1, id2, bodyPartName2) {
        return calcDistance2D3D(this.getBody(id1).getBodyPart3D(bodyPartName1).position, this.getBody(id2).getBodyPart3D(bodyPartName2).position);
    }
...
class BodyStream extends EventTarget {
    setupStream() {
        ...
        const bodies$ = pairs$.pipe(map(pair => {
...
export { detectBodies, movementState, AnalyzeBodyMovement, BodyStream, Body, Bodies, bodyPartsList, bodyPartNames };
