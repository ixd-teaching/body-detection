...
async function run(status) {
    ...
    detectBodies(config, (e) => {
        ...
        e.detail.bodies.listOfBodies.forEach(body => {
            ...
            if (body.bodyParts3D) {
                body.bodyParts3D.forEach(bodyPart => console.log(`${bodyPart.name}, ${bodyPart.position.x}, ${bodyPart.position.y}, ${bodyPart.speed.absoluteSpeed}, ${bodyPart.confidenceScore}`));
            }
            else {
                body.bodyParts2D.forEach(bodyPart => console.log(`${bodyPart.name}, ${bodyPart.position.x}, ${bodyPart.position.y}, ${bodyPart.speed.absoluteSpeed}, ${bodyPart.confidenceScore}`));
            }
        });
    });
}
