...
const inputResolution = { width: 800, height: 600 }
...
// configure with a posenet, a DOM videoelement, and whether single or multiple bodies should be detected at a time 
...
class Bodies {
    ...
    constructor(prevDetectedBodies, currDetectedBodies) {
        this.#prevDetectedBodies = prevDetectedBodies.bodies
        this.#currDetectedBodies = currDetectedBodies.bodies
        this.#timeLapsed = currDetectedBodies.timestamp - prevDetectedBodies.timestamp
    }

    getNumOfBodies() {
        return this.#currDetectedBodies.length
    }

    getBodies() {
        const bodies = []
        for (let i = 0; i < this.getNumOfBodies(); i++)
            bodies.push(this.getBodyAt(i))
        return bodies
    }

    getBodyAt(index) {
        // we assume here that the index corresponds to the same detected body among currently and previously detected bodies
        // this assumption might not always hold  
        const currBody = (index < this.#currDetectedBodies.length) ? this.#currDetectedBodies[index] : null
        const prevBody = (index < this.#prevDetectedBodies.length) ? this.#prevDetectedBodies[index] : null
        if (currBody)
...
// helper function that translate posenet data into an array of (BodyData) a format more friendly to work
function constructBody(currentBody, previousBody, timeLapsed) {
    ...
    currentBody.keypoints.forEach((curr, i) => {
        ...
        curr.position = {x: Math.round(curr.position.x), y: Math.round(curr.position.y)}
        const bodyPart = new BodyPart(curr.part, curr.position, speed, curr.score.toFixed(2))
    ...
    async run() {
        if (this.canRun && this.doRun) {
            ...
            if (this.detectionType == detectionType.multipleBodies)
                poses = await this.net.estimateMultiplePoses(this.videoElement, poseEstimationConfig)
            else
                poses = [await this.net.estimateSinglePose(this.videoElement, poseEstimationConfig)]
            ...
            if (poses.length > 0)
                this.dispatchEvent(new CustomEvent('bodiesDetected', {
                    detail: {
                        bodies: poses, // we refer to 'bodies' instead of 'poses' in the rest of the code
                        timestamp: timestamp
                    }
                }))
