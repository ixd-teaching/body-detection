    ...
    currentBody.keypoints.forEach((curr, i) => {
        ...
        if (hasPreviousBody) {
          ...
          speed = {
            ...
            absoluteSpeed: Math.sqrt(Math.pow(speedX, 2)+Math.pow(speedY, 2))}
        }
...
// A 'pose' event is fired with posedata in the shape of a Bodies object when one or more poses are detected
...
// helper function that translate posenet data into an array of (BodyData) a format more friendly to work with
    /* The format consists of an array of one or more bodies (BodyData) detected at a time.BodyData represents the two latest samples (current, previous) of bodyposes, but the current sample defines the pose. The previous sample is only used for calculating changes in pose. Sometimes it can happen that the number of bodies detected changes between samples (a body may have appeared or disappeared). In those cases it is again the latest sample that is authoratitative and defines how many bodies are detected. */   
